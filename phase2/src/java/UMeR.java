...
public class UMeR implements Serializable {
    ...
    public boolean addClient(String email, String name, String password, String address, LocalDate date){
        if (email != null && name != null && password != null && address != null && date != null &&
            ...
            return this.registerUser(client, null);
        }
        else return false;
    }
    ...
    public boolean addDriver(String email, String name, String password, String address, LocalDate date, String company){
        if (email != null && name != null && password != null && address != null && date != null &&
            ...
            return this.registerUser(driver, company);
        }
        else return false;
    }
    ...
    public boolean addCompany(String name, String password){
        if (name != null && password != null && !name.equals("") && !password.equals("") && !name.equals("admin"))
            return this.registerCompany(name, password);
        else return false;
    }

    public boolean addVehicle(String user, String licencePlate, String vehicleClass, String condition, Point2D.Double position,String cl){
        if (licencePlate != null && vehicleClass != null && condition != null && position != null &&
            ...
            Vehicle vehicle = null;
            if (vehicleClass.equals("Carro"))       vehicle = new Car(licencePlate, reliable, position, user);
            if (vehicleClass.equals("Mota"))        vehicle = new Bike(licencePlate, reliable, position, user);
            if (vehicleClass.equals("Carrinha"))    vehicle = new Van(licencePlate, reliable, position, user);
            if (vehicleClass.equals("Helicóptero")) vehicle = new Helicopter(licencePlate, reliable, position, user);

            if (cl.equals("Driver")) {
                if (this.registerVehicleP(vehicle)) {
                    this.changeDriverVehicle(user, licencePlate);
                    return true;
                }
                else return false;
            }
            else return this.registerCompanyVehicle(user, vehicle);
        }
        else return false;
    }
    ...
    public Trip newTrip(Client client, String d, Point2D.Double start, Point2D.Double end){
        ...
        if (d.equals("--condutor-mais-próximo--")){
            Trip t = this.newTripClosest(client, end);
            if (t != null) {
                String vehicle = this.closestAvailableTaxi(client);
            ...
            Trip t = this.newTripSpecific(client, d, end);
            if (t == null) this.driverAvailiable = false;
            else this.driverAvailiable = true;
            if (this.getAllDrivers().get(d) == null) this.noAvailiableTaxis = true;
